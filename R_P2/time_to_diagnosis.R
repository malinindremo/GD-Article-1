...
time_to_diagnosis_1p_diagnosis <- function(d){
  pd <- d[
    excluded %in% c("No") &
  ...
  SaveA4(
    ...
    filename = fs::path(
      ...
      "descriptives",
      "1p_diag_time_to_hormones_surgery_survival.png"
    ))
  ...
  SaveA4(
    ...
    filename = fs::path(
      org::project$results_today,
      "descriptives",
      "1p_diag_time_to_X.png"
    ))
  
}

time_to_diagnosis_4p_diagnosis <- function(d){
  pd <- d[
    excluded %in% c("No") &
      dateFirst_F64_089 >= "2006-01-01" & 
      dateFirst_F64_089 <= "2015-12-31" & 
  ...
  q <- q + scale_y_continuous("P(receiving surgery/hormones)",lim=c(0,1),expand=c(0,0))
  q <- q + scale_x_continuous("Years from first F64.0/8/9 diagnosis",lim=c(0,5))
  q <- q + labs(caption="\nKaplan-Meier analysis restricted to a) first F64.0/8/9 diagnosis between 2006-01-01 and 2015-12-31, and b) at least 4 F64.0/8/9 diagnoses. Follow-up ends 2016-12-31.")
  q <- q + theme_fhi_lines()
  SaveA4(
    q, 
    filename = fs::path(
      org::project$results_today,
      "descriptives",
      "4p_diag_time_to_hormones_surgery_survival.png"
    ))
}
